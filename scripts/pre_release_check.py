...
def check_file_content():
    ...
    if contrib_path.exists():
        ...
        if "yourname" in content:
            ...
            print(
                f"  {Colors.RED}[X]{Colors.RESET} docs/CONTRIBUTING.md contains 'yourname' placeholder"
            )
        else:
            print(
                f"  {Colors.GREEN}[OK]{Colors.RESET} docs/CONTRIBUTING.md - no placeholders found"
            )
...
def main():
    ...
    if all(results):
        print(
            f"{Colors.GREEN}{Colors.BOLD}[OK] All checks passed! Ready for release.{Colors.RESET}"
        )
        print(f"\n{Colors.BLUE}Next steps:{Colors.RESET}")
        print(f"  1. Review changes: {Colors.YELLOW}git status{Colors.RESET}")
        print(
            f"  2. Commit changes: {Colors.YELLOW}git add . && git commit -m 'Prepare for v0.1.0 release'{Colors.RESET}"
        )
        print(
            f"  3. Tag release: {Colors.YELLOW}git tag -a v0.1.0 -m 'CMakeHub v0.1.0'{Colors.RESET}"
        )
        ...
        print(
            f"{Colors.RED}{Colors.BOLD}[X] Some checks failed! Please fix the issues above.{Colors.RESET}"
        )
        return 1
